...
import { availableTaxYears, LATEST_TAX_YEAR } from "@/lib/tax";
import { useLanguage, interpolate } from "@/lib/i18n";

export default function SalaryTaxCalculatorClient() {
  const { lang, t } = useLanguage();
  const s = t.salaryTaxCalc;

  const [financialYear, setFinancialYear] = useState(LATEST_TAX_YEAR);
    ...
    <div className="relative overflow-hidden bg-wash text-ink min-h-screen">
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6">
            ...
            <div className="rounded-2xl border border-rule bg-paper p-6 sm:p-7 shadow-xs space-y-6">
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs font-bold text-muted">{s.financialYearLabel}</span>
                  {availableTaxYears.map((year) => (
                    <button
                      key={year}
                      type="button"
                      onClick={() => setFinancialYear(year)}
                      className={`rounded-md px-2.5 py-1 text-xs font-bold cursor-pointer border ${
                        financialYear === year
        ...
        <div className="mt-6 mb-16 rounded-2xl border border-rule bg-paper p-6 sm:p-8">
          <h2 className="text-lg font-black text-ink mb-1">{s.slabBreakdownTitle}</h2>
          <p className="text-xs text-muted mb-6">{s.slabBreakdownSubtitle}</p>

          <div className="grid gap-8 sm:grid-cols-2">
            {[
              { label: s.oldRegimeLabel, breakdown: result.oldRegime.breakdown },
              { label: s.newRegimeLabel, breakdown: result.newRegime.breakdown },
            ].map((regime) => (
              <div key={regime.label}>
                <h3 className="text-xs font-extrabold uppercase tracking-widest text-muted mb-3">
                  {regime.label}
                </h3>
                <table className="w-full text-left text-sm">
                  <thead>
                    <tr className="border-b-2 border-rule text-[11px] font-extrabold uppercase text-muted">
                      <th className="py-2 pr-2">{s.colSlab}</th>
                      <th className="py-2 pr-2">{s.colRate}</th>
                      <th className="py-2 text-right">{s.colTax}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-rule/70">
                    {regime.breakdown.map((row) => (
                      <tr key={row.slab}>
        ...
        {/* Financial year, assumptions & disclaimer */}
        <div className="mb-16 rounded-2xl border border-rule bg-paper p-6 sm:p-8 text-sm text-muted space-y-4">
          <div className="flex flex-wrap gap-x-6 gap-y-1 text-xs font-bold text-ink">
            <span>
              {s.financialYearLabel}: FY {result.financialYear}
            </span>
            <span>
              {s.lastUpdatedLabel}:{" "}
              {new Date(result.lastUpdated).toLocaleDateString(lang === "ta" ? "ta-IN" : "en-IN", {
                year: "numeric",
                month: "long",
                day: "numeric",
              })}
          ...
          <div>
            <h3 className="text-xs font-black uppercase tracking-wider text-ink mb-1.5">
              {s.assumptionsTitle}
            </h3>
            <p className="leading-relaxed">{s.assumptionsDesc}</p>
          </div>
          <div>
            <h3 className="text-xs font-black uppercase tracking-wider text-ink mb-1.5">
              {t.nav.disclaimerTitle}
            </h3>
            <p className="leading-relaxed">{s.disclaimer}</p>
          </div>
        </div>

        <FAQ items={s.faq} heading={t.nav.faqHeading} />

        <RelatedCalculators currentSlug="salary-tax" t={t} heading={t.nav.relatedCalculatorsHeading} />
